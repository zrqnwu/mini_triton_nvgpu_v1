...
- 这一节里的 runtime/NCU 数字只代表 2026-04-03 当时版本。
- 当前 exact-tile fair benchmark 已经不再落后于 Triton，因此这里的“当前慢点”结论不能直接套用到最新代码。
- 但“resource closure / shape closure 仍未完整”这个方向判断仍然有效。
...
- 这条判断只代表 2026-04-03 当时状态。
- 当前 `EpiloguePlan` 已改成按 accumulator ownership grid 分组 direct pack，不再以 `row-run + shared budget` 作为 direct/shared-pack 主判断。
- 本节以下内容保留为历史背景，不再代表当前代码现状。
...
- 这条判断对当前 exact-tile direct path 已经不成立。
- 现在 exact-tile cleanup 明确要求 `register-pack direct-global` mainline，不允许带 `boundary-aware/shared-pack` fallback 语义。
- 当前 shared-pack 只保留给 non-exact/general-shape 路径。
- 本节以下内容保留为历史背景，不再代表当前 exact-tile 主线现状。
...
- 这一条仍然成立。
- 当前 `ResourceClosurePlan` 依然偏轻量，仍主要围绕 direct/shared-pack 两档 tradeoff，尚未扩成 split-k/persistent/general fallback 的统一 fixed-point owner。
...
- general-shape 的 compile/test 路径已有进展，但完整 shape closure 和统一硬件验收还未收口。
